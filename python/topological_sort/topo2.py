def topologicalSort(jobs, deps):
	...
	return getOrderedList(jobGraph)
...
def getOrderedList(jobGraph):
	orderedJobs = []
	nodesWithNoPrereqs = list(
		filter(lambda node: node.numOfPrereqs == 0, jobGraph.nodes)
	)
	while len(nodesWithNoPrereqs):
		node = nodesWithNoPrereqs.pop()
		orderedJobs.append(node.job)
		removeDeps(node, nodesWithNoPrereqs)
	graphHasEdges = any(node.numOfPrereqs for node in jobGraph.nodes)
